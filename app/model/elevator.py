...
class Elevator:
    ...
    def move(self, target_floor: int, wait_time: float | int = SPEED):
        ...
        while self.state.floor != target_floor:
            if direction < 0:
                self._one_floor_down(wait_time=wait_time)
            if direction > 0:
                self._one_floor_up(wait_time=wait_time)
        self._wait_on_floor(wait_time=wait_time)
        self.state.move = 'STOP'
        self.move_coroutine_running = False

    def logic(self, pressed_buttons: list[DisplayButton]) -> ElevatorState:
        match self.state.move:
            ...
            case 'STOP':
                if pressed_buttons:
                    ...
                    self.move(target_floor=t_floor)
        return self.state
